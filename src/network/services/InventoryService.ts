...
export class InventoryService extends BaseService {
  ...
  async getProduct(productId: number): Promise<ApiResponse<ProductDetailResponse>> {
    return this.makeRequest(() => this.apiClient.get(`/api/product/${productId}`));
  }

  async getSlab(slabId: number): Promise<ApiResponse<SlabDetailResponse>> {
    return this.makeRequest(() => this.apiClient.get(`/api/slab/${slabId}`));
  }

  async getProductLandedCost(productId: number): Promise<ApiResponse<any>> {
    return this.makeRequest(() => this.apiClient.get(`/api/product/landedCost/${productId}`));
  }

  async getLandedCost(id: number): Promise<ApiResponse<any>> {
    return this.makeRequest(() => this.apiClient.get(`/api/landedCost/${id}`));
  }

  async toggleSlabOnHold(slabId: number, isHold: boolean): Promise<ApiResponse<MutationResponse>> {
    return this.makeRequest(() => this.apiClient.put(`/api/slab/${slabId}/hold`, { isHold }));
  }
  ...
  async updateSellingPrice(
    request: UpdateSellingPriceRequest,
