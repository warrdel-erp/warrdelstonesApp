...
export const BundleSlabsListScreen: React.FC<BundleSlabsListScreenProps> = props => {
  const { bundleId, inventory } = props.route.params;
  const [slabs, setSlabs] = React.useState<Slab[]>([]);
  const [loading, setLoading] = React.useState<boolean>(false);

  useEffect(() => {
    setSlabs(inventory.bundles.find(b => b.bundle === bundleId)?.slabs ?? []);
  }, [bundleId]);
