...
export const InventoryByBlockPage: React.FC = () => {
  ...
  const getInventory = async () => {
    try {
      ...
      const response = await services.inventory.getInventory({
        ...
        params: { page: 1, limit: 2000, categorization: 'BLOCK' },
      });
      if (response.success) {
          ...
          <Accordion contentStyle={{ gap: theme.spacing.xs }}>
            {item.blocks.map(block => (
              <TouchableOpacity
                ...
                onPress={() => {
                  NavigationService.navigate(ScreenId.BLOCK_SLAB_LIST, {
                    inventory: item,
                    blockId: block.block,
                  });
                }}>
                <LabelValue containerStyle={{ flex: 1 }} label={'Block:'} value={block.block} />
                <LabelValue
                  containerStyle={{ flex: 1 }}
                  label={'Units:'}
                  value={block?.slabs?.length || '0'}
                />
                <LabelValue
                  containerStyle={{ flex: 2 }}
                  label={'Qty:'}
                  value={`${block?.totalQuantity ?? '0'} ${item.uom?.code}`}
                />
  ...
  return (
    <View>
      <FlatList
        data={inventory.filter(i => i.blocks.length > 0)}
